...
@Component({
    selector: 'app-filter-inspection-entry',
    template: `
    <mat-card class="test-entry-card">
      <mat-card-header>
        <mat-card-title>Filter Inspection Test Entry</mat-card-title>
        <mat-card-subtitle>Particle Analysis and Filter Inspection</mat-card-subtitle>
      </mat-card-header>
      
      <mat-card-content>
        <form [formGroup]="filterInspectionForm" (ngSubmit)="onSubmit()">
      ...
      <mat-card-actions>
        <button mat-raised-button 
                color="primary" 
                (click)="onSubmit()"
                [disabled]="!filterInspectionForm.valid || !filterInspectionForm.get('isSelected')?.value"
                [loading]="isLoading()">
          <mat-icon>save</mat-icon>
          Save Filter Inspection Test
        </button>
        ...
        <button mat-button 
                (click)="onPartialSave()"
                [disabled]="!filterInspectionForm.get('isSelected')?.value || isLoading()">
...
export class FilterInspectionEntryComponent implements OnInit {
    ...
    // Form
    filterInspectionForm: FormGroup;

    constructor() {
        this.filterInspectionForm = this.fb.group({
    ...
    get particleTypesArray(): FormArray {
        return this.filterInspectionForm.get('particleTypes') as FormArray;
    }
    ...
    addAllComments(): void {
        ...
        this.filterInspectionForm.patchValue({
            mainComments: allComments
        });
    }

    onSubmit(): void {
        if (this.filterInspectionForm.valid && this.filterInspectionForm.get('isSelected')?.value) {
            this.saveTestResult('S'); // Saved status
        }
    }

    onPartialSave(): void {
        if (this.filterInspectionForm.get('isSelected')?.value) {
            this.saveTestResult('A'); // Accepted status for partial save
        }
    }

    onClear(): void {
        this.filterInspectionForm.reset({
            overallSeverity: '',
            mainComments: '',
            isSelected: true
        });
    ...
    private saveTestResult(status: string): void {
        if (!this.filterInspectionForm.valid) return;

        const formValue = this.filterInspectionForm.value;
        ...
        this.testResultService.saveTestResults({
            ...
            next: (result) => {
                ...
                this.snackBar.open('Filter Inspection test saved successfully', 'Close', {
                    duration: 3000,
            ...
            error: (error) => {
                this.isLoading.set(false);
                this.snackBar.open('Error saving Filter Inspection test: ' + error.message, 'Close', {
                    duration: 5000,
                    horizontalPosition: 'end',
                    verticalPosition: 'top'
                });
